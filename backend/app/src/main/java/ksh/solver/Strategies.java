...
public final class Strategies {
    ...
    private static Step ruleOf45(final Cage[] cages, final Cell[][] grid) {
        ...
        for (int i = 0; i < 9; i++) {
            rows[i] = new Pair<>(45, Arrays.asList(Cage.rowCages[i].getCells()));
            columns[i] = new Pair<>(45, Arrays.asList(Cage.columnCages[i].getCells()));
            nonets[i] = new Pair<>(45, Arrays.asList(Cage.nonetCages[i].getCells()));
        }
